...
export default function SocialHandlesRail({ socialResult, isLoading, onAffiliateClick }: SocialHandlesRailProps) {
  ...
  const priorityPlatforms = ['instagram', 'twitter', 'tiktok', 'threads', 'linkedin', 'youtube', 'reddit', 'twitch', 'medium', 'github'];
  ...
  const getPlatformIcon = (platform: string) => {
    switch (platform) {
      ...
      case 'threads':
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.845 1.205 8.598.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.29 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.018-5.11.882-6.545 2.568-1.356 1.597-2.046 3.881-2.049 6.787.003 2.907.697 5.186 2.061 6.78 1.443 1.684 3.647 2.538 6.552 2.54 4.41-.02 7.083-2.013 8.192-6.115l.01-.043c.298-1.34.233-2.67-.189-3.97-.381-1.175-1.034-2.167-1.892-2.873-1.088-.899-2.43-1.354-3.997-1.354-1.188 0-2.193.248-2.995.738-.737.449-1.3 1.083-1.67 1.884-.37-.601-.854-1.083-1.443-1.434-.713-.425-1.551-.636-2.489-.636-1.091 0-2.023.3-2.767.89-.66.523-1.174 1.275-1.487 2.179-.347.999-.524 2.152-.524 3.438 0 1.287.177 2.443.524 3.442.313.907.827 1.661 1.487 2.187.744.593 1.676.89 2.767.89 1.447 0 2.582-.487 3.374-1.447l.078.11c.451 1.067 1.297 1.957 2.366 2.51 1.032.533 2.23.803 3.564.803 2.1 0 3.885-.661 5.306-1.96 1.42-1.301 2.4-3.14 2.91-5.467l2.04.569c-.571 2.603-1.753 4.8-3.516 6.531-1.762 1.73-3.925 2.607-6.437 2.607-1.534 0-2.91-.311-4.095-. 927-1.138.592-2.052 1.074-2.717 1.434-.665.36-1.447.54-2.33.54z"/>
          </svg>
        );
      case 'reddit':
        return (
  ...
  const getRegistrationUrl = (platform: string) => {
      ...
      {onAffiliateClick && (
        <div className="mb-6">
          {availableCount >= totalCount * 0.4 ? (
            <button
              onClick={() => {
                const hootsuiteUrl = 'https://www.hootsuite.com/plans';
                if (onAffiliateClick) {
                  onAffiliateClick('hootsuite', 'social-management', hootsuiteUrl);
                }
                window.open(hootsuiteUrl, '_blank');
              }}
